/** Build reviewable deployment transactions without credentials, RPC calls or deployment-record writes. */
...
for (const chainId of plan.chains) {
    ...
    const deployment = await buildProxyDeployment({...args,eas,initialOwner:plan.owner,initialAttester:plan.initialAttester});
    ...
    const output = { chainId, contract:'SanctionsResolverV2', status:'prepared-not-deployed', address, schemaUID,
        proxyType:'uups', implementation:deployment.implementation, implementationSalt:deployment.implementationSalt,
        ...
        transactions:[{purpose:'Deploy SanctionsResolverV2 implementation',from:plan.deployer,to:plan.createx,value:'0',data:implementationData},
            {purpose:'Deploy and initialize ERC1967Proxy',from:plan.deployer,to:plan.createx,value:'0',data:deployData},
