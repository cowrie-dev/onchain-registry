...
import { buildResolverInitCode, computeCreate3Address, computeCreate3ProxyAddress, computeGuardedSalt, CREATEX_DEPLOY_CREATE3_ABI } from './utils/createx.js';
...
const plan = JSON.parse(await readFile('deployment-plans/sanctions-v2.json', 'utf8')) as {
    address: Address; salt: Hex; deployer: Address; owner: Address; initialAttester: Address; createx: Address; chains: number[];
};
...
for (const chainId of plan.chains) {
    ...
    const initCode = await buildResolverInitCode({eas,initialOwner:plan.owner,initialAttester:plan.initialAttester});
    ...
    const output = { chainId, contract:'SanctionsResolverV2', status:'prepared-not-deployed', address, schemaUID,
        ...
        transactions:[{purpose:'Deploy SanctionsResolverV2',from:plan.deployer,to:plan.createx,value:'0',data:deployData},
...
console.log(JSON.stringify({ address, schemaUID, proxy:computeCreate3ProxyAddress(args), guardedSalt:computeGuardedSalt(plan.deployer,plan.salt),chains:plan.chains },null,2));
